...
class autoencoder:
    ...
    def train(self, batch_size, inputs, labels, validation_input, validation_targets, num_epochs, learning_rate):
        ...
        for j in range(num_epochs):
            ...
            while i+batch_size != len(inputs):
                label_list.append(labels[i:i+batch_size])
                # print(i+batch_size)
        ...
        with open('autoencoder_loss_list.pickle', 'wb') as wf:
    ...
    def forward_pass(self, inputs):
        ...
        for i in range(self.num_layers-1):
            ...
            elif self.layers[i+1].activation_function == "sigmoid":
    ...
    def calculate_error(self, labels):
        if self.cost_function == "mean_squared":
            # print("----- loss ----")
            self.error = np.square(np.subtract(labels, self.layers[self.num_layers-1].activations)).mean()
    ...
    def back_pass(self, labels):
        ...
        # deltaw = np.matmul(np.asarray(self.layers[i-1].activations).transpose(), deltab)
    ...
    def get_encoder(self, testing_data):
        # print(len(input))
        self.forward_pass(testing_data)
        encode = self.layers[1].activations
        reconstruct = self.layers[-1].activations

        for i in range(self.num_layers):
            print("------- layer" + repr(i))
            print(np.shape(self.layers[i].weights_for_layer))

        with open('weight.pickle', 'wb') as wf:
            pickle.dump(self.layers[1].weights_for_layer, wf)

        with open('encoder.pickle', 'wb') as wf:
            pickle.dump(encode, wf)

        print(np.shape(self.layers[-1].activations))
        with open('autoencoder_result.pickle', 'wb') as wf:
            pickle.dump(reconstruct, wf)
